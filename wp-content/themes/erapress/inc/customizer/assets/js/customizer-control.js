...
( function( $ ) {
	...
	ERAPRESSCustomizer = {
		...
		init: function()
		{
			ERAPRESSCustomizer._initToggles();
		},
		...
		_initToggles: function()
		{
			...
			ERAPRESSControlTrigger.triggerHook( 'erapress-toggle-control', api );

			// Loop through each setting.
			$.each( ERAPRESSCustomizerToggles, function( settingId, toggles ) {
						...
						$.each(toggles, function( i, toggle) {
							...
							$.each(toggle.controls, function( k, controlId) {
								...
								if ( typeof parent != 'undefined' ) {
									...
									if ( ! visibleIt ) {
										$( '.control-section[aria-owns="' + parentId + '"]' ).addClass( 'erapress-hide' );
									} else {
										$( '.control-section[aria-owns="' + parentId + '"]' ).removeClass( 'erapress-hide' );
									}
								}
							});
						});
	...
	$( function() { ERAPRESSCustomizer.init(); } );
